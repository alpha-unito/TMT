...
void BaseHandler::stop() {
    ...
    if (rb1_) {
        ring_buffer__free(rb1_);
        rb1_ = nullptr;
    }
    if (rb2_) {
        ring_buffer__free(rb2_);
        rb2_ = nullptr;
    }
}

void BaseHandler::drain_until(uint64_t total_expected) const {
    constexpr int MAX_IDLE = 5000;
    int idle = 0;
...
void BaseHandler::set_ring_buffers(ring_buffer *rb1, ring_buffer *rb2) {
    rb1_ = rb1;
    rb2_ = rb2;
}

int BaseHandler::set_cfg_enabled_map(const int fd) {
    uint32_t key = 0, one = 1;
    return bpf_map_update_elem(fd, &key, &one, BPF_ANY);
}

int BaseHandler::freeze_cfg_enabled_map(const int fd) {
    uint32_t key = 0, zero = 0;
    return bpf_map_update_elem(fd, &key, &zero, BPF_ANY);
}

uint64_t BaseHandler::snapshot_evcount_percpu(const int fd) const {
...
std::string BaseHandler::human_ts(uint64_t ts_ns) {
    struct sysinfo si{};
    sysinfo(&si);
    time_t now = time(nullptr), boot = now - si.uptime;
