...
static int sample_cb(void *ctx, void *data, size_t len) {
    const auto *c = static_cast<RbCtx *>(ctx);
...
bool ExecveHandler::install() {
    ...
    const bpf_program *enter_prog = bpf_object__find_program_by_name(obj_, "trace_execve");
    const bpf_program *exit_prog = bpf_object__find_program_by_name(obj_, "trace_execve_exit");
    ...
    rb1_ = ring_buffer__new(map_rb_in_, sample_cb, &rb_in_ctx_, /*opts*/nullptr);
    rb2_ = ring_buffer__new(map_rb_out_, sample_cb, &rb_out_ctx_, /*opts*/nullptr);
    if (!rb1_ || !rb2_) {
...
int ExecveHandler::on_sample_with_tag(const char *tag, const void *data, const size_t len) {
